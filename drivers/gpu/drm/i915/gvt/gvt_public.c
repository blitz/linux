...
{
	BUG_ON(port >= ARRAY_SIZE(vgpu->display.ports));
	return &(vgpu->display.ports[port]);
}
EXPORT_SYMBOL_GPL(intel_vgpu_port);

struct drm_device *intel_vgpu_drm(struct intel_vgpu *vgpu)
{
	return &vgpu->gvt->dev_priv->drm;
}
EXPORT_SYMBOL_GPL(intel_vgpu_drm);

void *intel_vgpu_edid_block(struct intel_vgpu_port *port)
{
	return port->edid ? port->edid->edid_block : NULL;
}

...
{
	struct attribute_group **svp_vgpu_type_groups;
	char type_name[64];
	int desired_type = 0;
	int max_type = 0;
	int i;
	...
	if (!intel_gvt_ops->get_gvt_attrs(&svp_vgpu_type_groups))
		goto err;

	/* Find largest available type. */
	for (i = 0; svp_vgpu_type_groups[i] != NULL; i++) {
		max_type = i;
	}
	...
	desired_type = (desired_type >= max_type) ? max_type - 1 : desired_type;
	...
	snprintf(type_name, sizeof(type_name), "i915-%s",
		 svp_vgpu_type_groups[desired_type]->name);

	return intel_gvt_ops->gvt_find_vgpu_type(gvt, type_name);
...
static void slvmgt_host_exit(struct device *dev)
{
...
{
	if (intel_gvt_register_hypervisor(&slvmgt_mpt)) {
		return -ENODEV;
	}
